...
> **For agentic workers:** REQUIRED SUB-SKILL: Execute this plan task-by-task inline; subagents are not authorized for this run. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**Interfaces:**
- Produces: `GET /api/v1/review-queue`, `ApiError`, role-aligned navigation.

- [ ] 编写纯 reviewer 读取审核队列及禁止模型访问的失败测试。
- [ ] 实现审核专用有界查询并复用批次响应转换。
- [ ] 用 `ApiError` 区分 401、403、404、429、5xx，移除业务页面的静默空数据回退。
- [ ] 对齐导航角色并添加页面级错误恢复。
- [ ] 运行 review 后端测试与前端 API 客户端测试。
...
- [ ] 编写更新、启停、轮换密钥和审计脱敏测试。
- [ ] 实现 `ModelUpdate` 和服务端重新加密，空密钥保持旧值。
- [ ] 为创建、更新、测试连接写入脱敏审计。
- [ ] 在模型中心增加编辑、启停、测试和明确错误反馈。
- [ ] 运行模型后端及前端针对性测试。
...
- [ ] 编写通过临时 API 服务执行清单并生成 JSONL 的失败测试。
- [ ] 实现登录、素材创建、任务提交、轮询、证据读取和结果落盘。
- [ ] 增加技术规则、人工试穿和分品类统计；缺少人工结果时保持未完成状态。
- [ ] 验证无密钥、缺素材、超时和部分失败不会生成虚假通过。
- [ ] 运行 benchmark 测试。
...
- [ ] 编写并发上限、重试退避、恢复去重和依赖失败健康测试。
- [ ] 增加任务调度字段和迁移。
- [ ] 实现固定任务集合、错误分类和指数退避。
- [ ] 实现 Redis TTL 去重唤醒与 Worker 心跳。
- [ ] 实现 liveness/readiness 并接入 Compose。
- [ ] 运行 worker、迁移和健康测试。
...
- [ ] 编写默认上限、翻页稳定性和批量提交快速返回测试。
- [ ] 将批次最新 Step 查询合并为单次 SQL。
- [ ] 为主要列表增加上限和筛选，并保持旧响应兼容。
- [ ] 把批量行处理移动到 Worker 可恢复任务。
- [ ] 将运营汇总尽可能下推 SQL，并为资产响应增加条件请求支持。
- [ ] 运行列表、批量和运营测试。
...
- [ ] 编写生产默认密钥拒绝、Provider URL 策略和规则元数据测试。
- [ ] 增加生产配置校验与显式私网 Provider 开关。
- [ ] 补齐规则来源、生效时间、适用区域和复核状态。
- [ ] 配置安全响应头、非 root 容器、深度健康检查和资源限制。
- [ ] 增加 CI、备份和恢复校验脚本。
- [ ] 运行配置、模板、Compose config 和脚本静态检查。
...
- [ ] 编写角色导航、退出、审核标签和错误状态前端测试。
- [ ] 实现移动端菜单、当前页状态、跳到主内容和退出登录。
- [ ] 让系统状态读取 readiness，并提供不可用说明。
- [ ] 补齐审核标签、动态播报、焦点样式和画布替代路径。
- [ ] 运行 Vitest、ESLint、构建和一次 Impeccable detector。
...
- [ ] 更新部署、模型轮换、Benchmark、备份恢复和外部验收文档。
- [ ] 运行后端完整测试与 Ruff。
- [ ] 运行前端完整测试、ESLint、TypeScript 和生产构建。
- [ ] 启动 Compose，检查迁移、readiness、MinIO、Worker 和 Playwright。
- [ ] 生成 1440px 与 375px 截图，批量修复后最多复核一次。
- [ ] 提交功能分支，合并 main，验证 main 并按仓库规则处理推送与分支清理。
