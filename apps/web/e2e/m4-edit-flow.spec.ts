...
test("operator branches, masks, edits, composes, reviews, and exports one image", async ({
  page,
}) => {
  ...
  const productValue = await page
    .getByLabel("商品")
    .locator("option")
    .filter({ hasText: `M4-E2E-${suffix}` })
    .getAttribute("value");
  const modelValue = await page
    .getByLabel("生图模型")
    .locator("option")
    .filter({ hasText: `M4 编辑 Mock ${suffix}` })
    .getAttribute("value");
  ...
  await page
    .getByLabel("修改要求")
    .fill("只替换选区背景为柔和米色，商品主体保持不变");
  await page.getByRole("button", { name: "生成编辑版本" }).click();
  await expect(
    page.getByText("编辑任务已创建，正在生成和质检。"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("版式编辑已生成新版本并进入审核。"),
  ).toBeVisible();
