...
test("operator compiles, generates, quality-checks, and exports a platform image", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "M2 平台套图测试衬衫" }),
  ).toBeVisible();
  ...
  await page
    .getByLabel("目标平台")
    .selectOption({ label: "Amazon（全球默认） · v1" });
  await page.getByRole("button", { name: "编译图片套装" }).click();
  await expect(
    page.getByRole("heading", { name: "逐图核对与覆盖 · 3 张" }),
  ).toBeVisible();
  await expect(
    page.locator("summary").filter({ hasText: "正面主图" }),
  ).toBeVisible();
  await expect(
    page.locator("summary").filter({ hasText: "材质详情" }),
  ).toBeVisible();
