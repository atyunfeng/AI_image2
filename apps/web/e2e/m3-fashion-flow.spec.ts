...
async function uploadCurrentView(page: import("@playwright/test").Page, view: string, name: string, expectedCount: number) {
  ...
  await expect(page.locator('img[alt$="模特参考图"]')).toHaveCount(expectedCount);
}

test("operator creates authorized talent, multi-angle images, virtual try-on, and exports", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "M3 多角度试穿外套" })).toBeVisible();
  const productUrl = page.url();
  for (const view of ["front"]) {
    await page.locator('select[name="view"]').selectOption(view);
    await page.locator('input[type="file"]').setInputFiles({ name: `product-${view}.png`, mimeType: "image/png", buffer: png });
    await page.getByRole("button", { name: "上传参考图" }).click();
    await expect(page.getByText("参考图已固化")).toBeVisible();
  }

  await page.goto("/fashion/new");
  let productOption = await page.getByLabel("服装 / 鞋 / 帽商品").locator("option").filter({ hasText: `M3-E2E-${suffix}` }).getAttribute("value");
  let profileOption = await page.getByLabel("授权模特").locator("option").filter({ hasText: `M3 虚拟模特 ${suffix}` }).getAttribute("value");
  let modelOption = await page.getByLabel("生图模型").locator("option").filter({ hasText: `M3 全能力 Mock ${suffix}` }).getAttribute("value");
  ...
  await expect(page.getByRole("status")).toContainText("requires product reference: side");
  ...
  for (const view of ["side", "back"]) {
    ...
    await expect(page.getByText("参考图已固化")).toBeVisible();
    await page.reload();
    await expect(page.locator('img[alt$="参考图"]')).toHaveCount(view === "side" ? 2 : 3);
  }

  await page.goto("/fashion/new");
  productOption = await page.getByLabel("服装 / 鞋 / 帽商品").locator("option").filter({ hasText: `M3-E2E-${suffix}` }).getAttribute("value");
  profileOption = await page.getByLabel("授权模特").locator("option").filter({ hasText: `M3 虚拟模特 ${suffix}` }).getAttribute("value");
  modelOption = await page.getByLabel("生图模型").locator("option").filter({ hasText: `M3 全能力 Mock ${suffix}` }).getAttribute("value");
  await page.getByLabel("服装 / 鞋 / 帽商品").selectOption(productOption!);
  await page.getByLabel("授权模特").selectOption(profileOption!);
  await page.getByLabel("生图模型").selectOption(modelOption!);
  await page.getByText("模特侧面图").click();
