...
const pack = (id: string, name: string, kind: "category" | "brand") => ({
  id,
  version_id: `${id}-v1`,
  slug: id,
  name,
...
it("offers CSV preflight and execution inputs", () => {
  render(
    <BulkProduction
      models={[model]}
      packs={[
        pack("apparel", "服装", "category"),
        pack("neutral", "中性", "brand"),
      ]}
      initialJobs={[]}
    />,
  );
  expect(screen.getByLabelText("CSV 或 XLSX 文件")).toBeVisible();
  expect(screen.getByLabelText("生图模型")).toHaveValue("model-1");
  expect(screen.getByText("仅预检文件")).toBeVisible();
